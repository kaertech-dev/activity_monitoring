...
def fetch_operator_en_by_date(selected_date=None):
    ...
    cursor.close()
    conn.close()
    return all_data, selected_date

@app.get("/download-csv")
def download_csv():
    all_data, today_str = fetch_operator_en_by_date()
...
async def fetch_operator_en_by_date(request: Request):
    all_data, today_str = fetch_operator_en_by_date()
    columns = ['Customer', 'Project', 'operator_en','Target_Output', 'Current_Output', 'Process_Time(s)', 'Start_Time', 'End_time', 'Status']
    rows = [tuple(d[col] for col in columns) for d in all_data]
    return templates.TemplateResponse("monitoring_v3.html", {
        "request": request,
        "rows": rows,
        "columns": columns,
...
@app.get("/", response_class=HTMLResponse)
async def fetch_operator_en_by_date(request: Request, date: str=None):
    all_data, selected_date = fetch_operator_en_by_date(date)
    columns = ['Customer', 'Project', 'operator_en','Target_Output', 'Current_Output', 'Process_Time(s)', 'Start_Time', 'End_time', 'Status']
    rows = [tuple(d[col] for col in columns) for d in all_data]
    return templates.TemplateResponse("monitoring_v3.html", {
        "request": request,
